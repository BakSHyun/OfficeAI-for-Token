...
8. **모든 변경 후 검증 3종 실행:**
   ```bash
   npm run core:test   # 코어 로직 테스트 (28개+, 전부 통과해야 함)
   npm run lint        # ESLint
...
사용자 명령
  ...
  → orchestration/orchestrator.ts(DAG 동시 실행 + 재작업 루프)
      ├─ providers/*             (LLM 호출: openai/anthropic/codex-cli/mock)
...
| `core/src/providers/` | `LLMProvider` 구현 4종 + registry | 완성 |
...
- `tiers.<tier>.provider`: `openai` | `anthropic` | `codex-cli` | `mock`
...
- 내용:
  - 티어별 provider/model 표 (4행: local/economy/standard/premium). provider는 select(openai/anthropic/codex-cli/mock), model은 text input
...
유저 입장에서 **간편함·안정성·특별한 경험**이 판매의 조건이다. 아래 G 카드는 그 갭을 메우는 순서대로 정렬되어 있다. 각 카드는 카드 A~F와 같은 규칙(완료 기준 + 검증 3종)을 따른다.
